...
#define WITMAGIC_VELDECAY_DEF  0.99f    /* velocity decay per sample (0.9–0.999 typical) */
...
/* level: when sensor is flat (Z up), set accel offset so pitch/roll → 0 */
static void witmagic_level(t_witmagic *x) {
    x->vx = x->vy = x->vz = 0.f;
...
void witmagic_setup(void) {
    ...
    class_addmethod(witmagic_class, (t_method)witmagic_level, gensym("level"), 0);
